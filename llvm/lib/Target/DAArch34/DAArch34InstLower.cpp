...
static MCOperand lowerSymbolOperand(const MachineOperand &MO, MCSymbol *Sym,
  ...
  const MCExpr *ME =
      MCSymbolRefExpr::create(Sym, MCSymbolRefExpr::VariantKind::VK_None, Ctx);
...
bool lowerDAArch34MachineOperandToMCOperand(const MachineOperand &MO,
  ...
  default:
    report_fatal_error(
        "LowerDAArch34MachineOperandToMCOperand: unknown operand type");
  }
}
...
bool lowerDAArch34MachineInstrToMCInst(const MachineInstr *MI, MCInst &OutMI,
  ...
  for (const MachineOperand &MO : MI->operands()) {
    MCOperand MCOp;
    if (lowerDAArch34MachineOperandToMCOperand(MO, MCOp, AP)) {
